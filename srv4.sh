...
mv /tmp/webatom.green.skills.crt /etc/nginx/ssl/
mv /tmp/webatom.green.skills.key /etc/nginx/ssl/
mv /tmp/git.green.skills.crt /etc/nginx/ssl/
mv /tmp/git.green.skills.key /etc/nginx/ssl/
mv /tmp/GreenCA.crt /etc/nginx/ssl/
...
# Создать конфигурацию для webatom
cat > /etc/nginx/sites-available/webatom << 'EOF'
# Перенаправление HTTP → HTTPS
server {
    listen 80;
    server_name webatom.green.skills;
...
server {
    ...
    # SSL сертификаты
    ssl_certificate /etc/nginx/ssl/webatom.green.skills.crt;
    ssl_certificate_key /etc/nginx/ssl/webatom.green.skills.key;
    ...
    # Логи
    access_log /var/log/nginx/webatom_access.log;
    error_log /var/log/nginx/webatom_error.log;
    
    # Проксирование на SRV-WEB1
    location / {
...
# Активировать сайт
ln -s /etc/nginx/sites-available/webatom /etc/nginx/sites-enabled/
...
# Проверить
curl -k https://webatom.green.skills
curl http://10.20.0.12
...
CREATE USER gitflic_user WITH PASSWORD 'P@ssw0rd';

# Создать базу данных
CREATE DATABASE gitflic OWNER gitflic_user;

# Предоставить права
GRANT ALL PRIVILEGES ON DATABASE gitflic TO gitflic_user;
